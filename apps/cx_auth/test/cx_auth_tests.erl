...
platform_admin_token(Keypair) ->
    Token = cx_auth_test:token(Keypair, #{<<"sub">> => <<"boss">>,
                                          ?TENANT_CLAIM => <<"t1">>}),
...
user_token_role_permissions(Keypair) ->
    ...
    {ok, #{<<"id">> := RoleId}} =
        cx_role:create(Admin, #{<<"name">> => <<"Agent">>,
                                <<"permissions">> => [<<"agent:ready:self">>]}),
    {ok, #{<<"id">> := UserId}} =
        cx_user:create(Admin, #{<<"name">> => <<"A">>, <<"email">> => <<"a@x">>,
                                <<"subject">> => <<"agent-sub">>,
                                <<"role_ids">> => [RoleId]}),
    Token = cx_auth_test:token(Keypair, #{<<"sub">> => <<"agent-sub">>,
                                          ?TENANT_CLAIM => T}),
...
unknown_subject(Keypair) ->
    Token = cx_auth_test:token(Keypair, #{<<"sub">> => <<"nobody">>,
...
disabled_user(Keypair) ->
    ...
    Admin = cx_authz:ctx(T, [<<"*">>]),
    {ok, #{<<"id">> := UserId}} =
        cx_user:create(Admin, #{<<"name">> => <<"D">>, <<"email">> => <<"d@x">>,
                                <<"subject">> => <<"disabled-sub">>}),
    {ok, _} = cx_user:update(Admin, UserId, #{<<"status">> => <<"disabled">>}),
    Token = cx_auth_test:token(Keypair, #{<<"sub">> => <<"disabled-sub">>,
                                          ?TENANT_CLAIM => T}),
    ?assertEqual({error, unauthorized}, cx_auth:authenticate(Token)).
...
crud_forbidden_without_permission(Keypair) ->
    ...
    Admin = cx_authz:ctx(T, [<<"*">>]),
    {ok, #{<<"id">> := _}} =
        cx_user:create(Admin, #{<<"name">> => <<"P">>, <<"email">> => <<"p@x">>,
                                <<"subject">> => <<"powerless">>}),
    Token = cx_auth_test:token(Keypair, #{<<"sub">> => <<"powerless">>,
                                          ?TENANT_CLAIM => T}),
    {ok, Ctx} = cx_auth:authenticate(Token),

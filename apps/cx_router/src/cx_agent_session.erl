...
start_link(TenantId, AgentId, Skills, Profile) ->
    gen_statem:start_link({via, cx_reg, {agent, TenantId, AgentId}},
                          ?MODULE, [TenantId, AgentId, Skills, Profile], []).

callback_mode() -> handle_event_function.

init([TenantId, AgentId, Skills, Profile]) ->
    Data = #sess{tenant = TenantId, agent_id = AgentId,
...
handle_event({call, From}, {set_ready, Media, NewState}, _State, Data) ->
    ...
    publish(Data1, undefined, Media, agent_ready_changed,
            #{<<"media_type_id">> => Media,
...
handle_event({call, From}, {offer, Offer}, online, Data) ->
    #{offer_id := OfferId, interaction_id := IId, media := Media,
      queue_key := QueueKey, queue_pid := QueuePid} = Offer,
    Routable = maps:get(Media, Data#sess.ready, undefined) =:= ready
        andalso cx_routing:can_route(Data#sess.profile, mix_of(Data), Media),
    case Routable of
        true ->
            MonRef = erlang:monitor(process, QueuePid),
            Pending = maps:put(OfferId, {IId, Media, QueueKey, QueuePid, MonRef},
                               Data#sess.pending),
...
handle_event(cast, {offer_accepted, OfferId}, _State, Data) ->
    case maps:take(OfferId, Data#sess.pending) of
        {{IId, Media, QueueKey, _QueuePid, MonRef}, Pending} ->
            ...
            Data1 = Data#sess{pending = Pending,
                              active = maps:put(IId, {Media, QueueKey},
                                                Data#sess.active)},
            write_presence(Data1),
            {keep_state, Data1};
        error ->
...
handle_event({call, From}, {complete, IId}, State, Data) ->
    case maps:take(IId, Data#sess.active) of
        {{Media, QueueKey = {_Tenant, QueueId}}, Active} ->
            ...
            publish(Data, QueueId, Media, interaction_completed,
                    #{<<"interaction_id">> => IId,
            ...
            case WrapupMs > 0 of
                true ->
                    ...
                    publish(Data2, QueueId, Media, wrapup_started,
...
handle_event({call, From}, {extend_wrapup, Ms}, wrap_up, Data) ->
    ...
    write_presence(Data1),
    publish(Data1, undefined, undefined, wrapup_extended,
...
handle_event({call, From}, cancel_wrapup, wrap_up, Data) ->
    ...
    write_presence(Data1),
    publish(Data1, undefined, undefined, wrapup_cancelled,
            #{<<"agent_id">> => Data1#sess.agent_id}),
    cx_router_signal:agent_available(Data1#sess.tenant),
    {next_state, online, Data1,
     [{reply, From, ok}, {{timeout, wrapup}, cancel}]};
...
handle_event({timeout, wrapup}, expire, wrap_up, Data) ->
    Data1 = Data#sess{wrapup_until = 0},
    write_presence(Data1),
    publish(Data1, undefined, undefined, wrapup_ended,
            #{<<"agent_id">> => Data1#sess.agent_id}),
    cx_router_signal:agent_available(Data1#sess.tenant),
    {next_state, online, Data1};
...
handle_event({call, From}, get_state, State, Data) ->
    Info = #{<<"agent_id">> => Data#sess.agent_id,
             <<"ready">> => maps:map(fun(_, V) -> ready_to_bin(V) end,
                                     Data#sess.ready),
    ...
    case maps:size(Data#sess.active) of
        0 ->
            ...
            maps:foreach(
                ...
                end, Data#sess.pending),
            publish(Data, undefined, undefined, session_ended, #{}),
            {stop_and_reply, normal, [{reply, From, ok}]};
        _ ->
            {keep_state_and_data,
             [{reply, From, {error, has_active_interactions}}]}
    end;
...
handle_event(info, {'DOWN', MonRef, process, _Pid, _Reason}, _State, Data) ->
    ...
    Pending = maps:filter(fun(_, {_, _, _, _, Ref}) -> Ref =/= MonRef end,
                          Data#sess.pending),
...
terminate(_Reason, _State, Data) ->
    try
        mnesia:dirty_delete(cx_agent_presence,
...
write_presence(Data = #sess{tenant = Tenant, agent_id = AgentId}) ->
    Rec = #cx_agent_presence{key = {Tenant, AgentId},
                             pid = self(),
    ...
    cx_store:tx(fun() ->
        case mnesia:read(cx_interaction, {Tenant, IId}) of
            [Rec = #cx_interaction{state = active}] ->
                mnesia:write(Rec#cx_interaction{state = completed,
                                                agent_id = AgentId,
...
publish(#sess{tenant = Tenant}, QueueId, Media, Type, ExtraData) ->
    cx_event:publish(Tenant, QueueId, Media,

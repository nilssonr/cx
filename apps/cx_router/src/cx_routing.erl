...
-export([can_route/3, effective_requirements/2, skill_match/2,
         routable/3, eligible/4, rank/2]).

-type mix() :: #{binary() => non_neg_integer()}.
-type reqs() :: [{SkillId :: binary(), MinRank :: pos_integer()}].
-type snapshot() :: #{agent_id := binary(),
                      pid := pid() | undefined,
...
can_route(#cx_routing_profile{max_total = MaxTotal, media_caps = Caps,
                              guards = Guards}, Mix, Media) ->
    Total = lists:sum(maps:values(Mix)),
    TotalOk = case MaxTotal of
        unlimited -> true;
        N -> Total + 1 =< N
    end,
    CapOk = case Caps of
...
routable(#{ready := Ready, wrapup_until := WrapupUntil,
           mix := Mix, profile := Profile}, Media, NowMs) ->
    maps:get(Media, Ready, undefined) =:= ready
        andalso WrapupUntil =< NowMs
        andalso can_route(Profile, Mix, Media).

-spec eligible(binary(), reqs(), [snapshot()], integer()) -> [snapshot()].
eligible(Media, Reqs, Snapshots, NowMs) ->
    [S || S = #{skills := Skills} <- Snapshots,

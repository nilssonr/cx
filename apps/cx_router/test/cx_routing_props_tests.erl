...
guard() ->
    ?LET({W, Gte, Block}, {media(), choose(1, 5), non_empty(list(media()))},
         #rp_guard{when_media = W, gte = Gte, block = lists:usort(Block)}).

profile() ->
    ?LET({MaxTotal, Caps, Guards},
...
%% enforces).
skill_req() ->
    ?LET({SkillId, Base, RawSteps},
         {oneof([<<"s1">>, <<"s2">>, <<"s3">>]), choose(1, 10),
         ...
         begin
             ...
             Ranks = lists:sublist(
                 ...
                 length(Afters)),
             #skill_req{skill_id = SkillId, min_rank = Base,
                        widening = lists:zip(Afters, Ranks)}
         end).

snapshot() ->
    ?LET({N, Ranks}, {choose(1, 1000000), vector(3, choose(0, 10))},
         begin
             Skills = maps:from_list(
                 [{S, R} || {S, R} <- lists:zip([<<"s1">>, <<"s2">>, <<"s3">>], Ranks),
                            R > 0]),
             #{agent_id => integer_to_binary(N), pid => undefined,
               ready => #{<<"open_media">> => ready},
               mix => #{}, wrapup_until => 0, skills => Skills,
               profile => #cx_routing_profile{key = {<<"t">>, <<"p">>},
                                              name = <<"p">>},
               idle_since => 0}
         end).
...
prop_deny_wins_guards() ->
    ?FORALL({P, Mix, Media, G}, {profile(), mix(), media(), guard()},
        begin
            With = P#cx_routing_profile{guards = [G | P#cx_routing_profile.guards]},
            not cx_routing:can_route(With, Mix, Media)
                orelse cx_routing:can_route(P, Mix, Media)
        end).

%% Lowering the total cap can only remove permissions.
prop_lower_total_cap_monotone() ->
    ?FORALL({P, Mix, Media, N1, N2},
            {profile(), mix(), media(), choose(1, 10), choose(1, 10)},
        begin
            [Lo, Hi] = lists:sort([N1, N2]),
            not cx_routing:can_route(P#cx_routing_profile{max_total = Lo}, Mix, Media)
                orelse cx_routing:can_route(P#cx_routing_profile{max_total = Hi}, Mix, Media)
        end).

%% A full agent (total >= max_total) can never be routed anything.
prop_total_cap_invariant() ->
    ?FORALL({P, Mix, Media, N}, {profile(), mix(), media(), choose(1, 10)},
        ?IMPLIES(lists:sum(maps:values(Mix)) >= N,
            not cx_routing:can_route(P#cx_routing_profile{max_total = N},
                                     Mix, Media))).
...
prop_eligible_superset_over_time() ->
    ?FORALL({Reqs, Snaps, T1, T2},
            {non_empty(list(skill_req())), non_empty(list(snapshot())),
             choose(0, 200000), choose(0, 200000)},
        begin
            [Lo, Hi] = lists:sort([T1, T2]),
            Media = <<"open_media">>,
            Early = cx_routing:eligible(
                        Media, cx_routing:effective_requirements(Reqs, Lo),
                        Snaps, 0),
            Late = cx_routing:eligible(
                        Media, cx_routing:effective_requirements(Reqs, Hi),
                        Snaps, 0),

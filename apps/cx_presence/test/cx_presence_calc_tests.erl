...
no_devices_is_offline_test() ->
    ?assertEqual(
        ...
        cx_presence_calc:effective(auto(), 0, 1000, 2000, ?THR)
    ).
...
offline_keeps_message_test() ->
    ...
    ?assertEqual(
        ...
        cx_presence_calc:effective(D, 0, 0, 1000, ?THR)
    ).
...
manual_wins_over_activity_test() ->
    ...
    ?assertEqual(
        ...
        cx_presence_calc:effective(D, 2, 1000, 1001, ?THR)
    ).
...
idle_at_threshold_is_away_test() ->
    ...
    ?assertEqual(
        ...
        cx_presence_calc:effective(auto(), 1, Now - ?THR, Now, ?THR)
    ),
    ?assertEqual(
        #{state => <<"online">>, message => undefined},
        cx_presence_calc:effective(auto(), 1, Now - ?THR + 1, Now, ?THR)
    ).
...
until_at_now_is_expired_test() ->
    ...
    ?assertEqual(
        #{state => <<"online">>, message => undefined},
        cx_presence_calc:effective(D, 1, 5000, 5000, ?THR)
    ),
    %% just before, it holds
    ?assertEqual(
        #{state => <<"busy">>, message => <<"m">>},
        cx_presence_calc:effective(D, 1, 4999, 4999, ?THR)
    ).
...
from_row_test() ->
    ?assertEqual(
        ...
        cx_presence_calc:from_row(undefined)
    ),
    Row = #cx_presence_decl{
        key = {<<"t">>, <<"u">>},
        manual_state = <<"busy">>,

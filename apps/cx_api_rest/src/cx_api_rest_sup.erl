...
init([]) ->
    ...
    TransOpts = #{socket_opts => [{port, Port}, {ip, Ip}],
                  num_acceptors => Acceptors},
    ProtoOpts = #{env => #{dispatch => cx_rest_routes:dispatch()},
                  middlewares => [cowboy_router, cx_rest_auth_mw,
                                  cowboy_handler]},
    %% ranch child spec directly under our tree: deterministic shutdown
    %% ordering, no detached listener
    Listener = ranch:child_spec(cx_http, ranch_tcp, TransOpts,

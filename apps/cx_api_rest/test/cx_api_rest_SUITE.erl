...
init_per_suite(Config) ->
    ...
    ok = application:set_env(cx_core, mnesia_dir,
...
admin_crud_roundtrip(Config) ->
    ...
    {200, #{<<"id">> := SkillId}} =
        req(Config, post, Base ++ "/skills", Admin,
            #{<<"name">> => <<"Permits">>,
              <<"levels">> => [#{<<"rank">> => 1, <<"name">> => <<"trainee">>},
                               #{<<"rank">> => 2, <<"name">> => <<"expert">>}]}),
    %% media type, queue, profile, reason
    {200, #{<<"id">> := MediaId}} =
        req(Config, post, Base ++ "/media-types", Admin,
            #{<<"name">> => <<"open_media">>}),
    {200, #{<<"id">> := QueueId}} =
        req(Config, post, Base ++ "/queues", Admin,
            #{<<"name">> => <<"Building permits">>,
              <<"skill_reqs">> => [#{<<"skill_id">> => SkillId,
                                     <<"min_rank">> => 1}]}),
    {200, #{<<"id">> := ProfileId, <<"max_total">> := 5}} =
        req(Config, post, Base ++ "/routing-profiles", Admin,
    ...
    {200, #{<<"wrapup_duration_ms">> := 5000}} =
        req(Config, put, Base ++ "/queues/" ++ binary_to_list(QueueId), Admin,
    ...
    {200, #{<<"id">> := RoleId}} =
        req(Config, post, Base ++ "/roles", Admin,
            #{<<"name">> => <<"Agent">>,
              <<"permissions">> => [<<"agent:session:self">>]}),
    {200, #{<<"id">> := _UserId}} =
        req(Config, post, Base ++ "/users", Admin,
            #{<<"name">> => <<"Robin">>, <<"email">> => <<"r@x.dev">>,
              <<"subject">> => <<"crud-agent">>,
              <<"role_ids">> => [RoleId],
              <<"skills">> => #{SkillId => 2},
...
cross_tenant_forbidden(Config) ->
    ...
    AdminT2 = user_token(Config, T2, <<"t2-admin">>, [<<"queues:read">>,
                                                      <<"queues:write">>]),
    Base1 = binary_to_list(<<"/api/v1/tenants/", T1/binary>>),
    {403, _} = req(Config, get, Base1 ++ "/queues", AdminT2),
    {403, _} = req(Config, post, Base1 ++ "/queues", AdminT2,
                   #{<<"name">> => <<"sneaky">>}),
    ok.
...
agent_open_media_flow(Config) ->
    ...
    Agent = user_token(Config, Tid, <<"flow-agent">>,
                       [<<"agent:session:self">>, <<"agent:ready:self">>,
                        <<"agent:offers:self">>, <<"agent:wrapup:self">>]),
    Integrator = user_token(Config, Tid, <<"flow-integrator">>,
                            [<<"interactions:create">>,
                             <<"interactions:cancel">>,
                             <<"interactions:read">>]),
    ...
    {204, _} = req(Config, post,
                   "/api/v1/agent/offers/" ++ binary_to_list(OfferId) ++ "/accept",
                   Agent, #{}),
    {200, #{<<"state">> := <<"active">>,
            <<"properties">> := #{<<"sap_case">> := <<"0815">>}}} =
        req(Config, get, "/api/v1/interactions/" ++ binary_to_list(IId),
            Integrator),
...
integrator_cancel_rules(Config) ->
    ...
    Integrator = user_token(Config, Tid, <<"c-int">>,
                            [<<"interactions:create">>,
                             <<"interactions:cancel">>,
                             <<"interactions:read">>]),
    ...
    {404, _} = req(Config, post, "/api/v1/interactions", Integrator,
                   #{<<"queue_id">> => <<"nope">>,
                     <<"media_type_id">> => MediaId}),
    {200, _} = req(Config, put, Base ++ "/queues/" ++ binary_to_list(QueueId),
                   Admin, #{<<"status">> => <<"closed">>}),
...
forbidden_without_permission(Config) ->
    ...
    Base = binary_to_list(<<"/api/v1/tenants/", Tid/binary>>),
    {403, _} = req(Config, post, Base ++ "/queues", Nobody,
                   #{<<"name">> => <<"q">>}),
    {403, _} = req(Config, get, Base ++ "/users", Nobody),
    {403, _} = req(Config, post, "/api/v1/interactions", Nobody,
...
boss_token(Config, TenantId) ->
    ...
    cx_auth_test:token(Keypair, #{<<"sub">> => <<"boss">>,
...
user_token(Config, TenantId, Subject, Perms) ->
    ...
    {200, #{<<"id">> := _}} =
        req(Config, post, Base ++ "/users", Admin,
            #{<<"name">> => Subject, <<"email">> => <<Subject/binary, "@x">>,
              <<"subject">> => Subject, <<"role_ids">> => [RoleId]}),
    Keypair = proplists:get_value(keypair, Config),
    cx_auth_test:token(Keypair, #{<<"sub">> => Subject,
                                  <<"urn:zitadel:iam:org:id">> => TenantId}).
...
poll_offer(Config, Agent, N) ->
    case req(Config, get, "/api/v1/agent/session", Agent) of
        {200, #{<<"pending_offers">> := [OfferId | _]}} -> {ok, OfferId};
        {200, _} -> timer:sleep(20), poll_offer(Config, Agent, N - 1)
    end.
...
req(Config, Method, Path, Token, Body, _Opts) ->
    ...
    {ok, {{_, Status, _}, _, RespBody}} =
        httpc:request(Method, Request, [{timeout, 15000}],
                      [{body_format, binary}]),
    Decoded = case cx_json:decode(RespBody) of
        {ok, Map} -> Map;
        {error, _} -> RespBody
    end,
    {Status, Decoded}.

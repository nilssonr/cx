...
tenant_explicit_id() ->
    Ctx = cx_authz:ctx(<<"platform">>, [<<"tenants:admin">>]),
    {ok, #{<<"id">> := <<"org-123">>}} =
        cx_tenant:create(Ctx, #{<<"name">> => <<"Mapped">>,
                                <<"id">> => <<"org-123">>}),
    ?assertEqual({error, already_exists},
                 cx_tenant:create(Ctx, #{<<"name">> => <<"Dup">>,
                                         <<"id">> => <<"org-123">>})),
...
user_crud() ->
    ...
    ?assertEqual({error, {invalid, <<"skills">>}},
...
role_crud() ->
    ...
    {ok, #{<<"id">> := Id, <<"permissions">> := [<<"queues:read">>]}} =
        cx_role:create(Ctx, #{<<"name">> => <<"Viewer">>,
...
skill_crud_and_level_validation() ->
    T = cx_id:new(),
    Ctx = admin(T),
    Levels = [#{<<"rank">> => 2, <<"name">> => <<"expert">>},
              #{<<"rank">> => 1, <<"name">> => <<"trainee">>}],
    {ok, #{<<"id">> := Id, <<"levels">> := Sorted}} =
        cx_skill:create(Ctx, #{<<"name">> => <<"Permits">>, <<"levels">> => Levels}),
    ?assertEqual([#{<<"rank">> => 1, <<"name">> => <<"trainee">>},
                  #{<<"rank">> => 2, <<"name">> => <<"expert">>}], Sorted),
    ?assertEqual({error, {invalid, <<"levels">>}},
                 cx_skill:update(Ctx, Id,
                     #{<<"levels">> => [#{<<"rank">> => 1, <<"name">> => <<"a">>},
                                        #{<<"rank">> => 1, <<"name">> => <<"b">>}]})),
...
routing_profile_crud() ->
    T = cx_id:new(),
    Ctx = admin(T),
    {ok, #{<<"id">> := Id, <<"max_total">> := 5,
           <<"guards">> := [#{<<"when_media">> := <<"voice">>}]}} =
        cx_routing_profile:create(Ctx,
            #{<<"name">> => <<"Default">>,
              <<"max_total">> => 5,
              <<"media_caps">> => #{<<"chat">> => 3},
              <<"guards">> => [#{<<"when_media">> => <<"voice">>, <<"gte">> => 1,
                                 <<"block">> => [<<"chat">>, <<"email">>]}]}),
    {ok, #{<<"max_total">> := null}} =
        cx_routing_profile:update(Ctx, Id, #{<<"max_total">> => null}),
    ?assertEqual({error, {invalid, <<"guards">>}},
                 cx_routing_profile:update(Ctx, Id,
                     #{<<"guards">> => [#{<<"when_media">> => <<"voice">>}]})),
...
permission_denied() ->
    T = cx_id:new(),
    NoPerms = cx_authz:ctx(T, []),
    ?assertEqual({error, forbidden}, cx_user:create(NoPerms, #{<<"name">> => <<"x">>, <<"email">> => <<"x@x">>})),

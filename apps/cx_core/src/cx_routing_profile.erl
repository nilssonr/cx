...
create(Ctx = #auth_ctx{tenant_id = T}, Params) ->
    maybe
        ...
        {ok, Guards} ?= case Params of
            #{<<"guards">> := Raw} -> parse_guards(Raw);
            _ -> {ok, []}
        end,
        Rec = #cx_routing_profile{key = {T, cx_id:new()}, name = Name,
                                  max_total = MaxTotal,
                                  media_caps = MediaCaps, guards = Guards},
...
update(Ctx = #auth_ctx{tenant_id = T}, ProfileId, Params) ->
    maybe
        ...
        {ok, Name} ?= cx_params:opt_bin(Params, <<"name">>,
        ...
        Rec = Rec0#cx_routing_profile{name = Name, max_total = MaxTotal,
                                      media_caps = MediaCaps, guards = Guards},
        ok = cx_store:tx(fun() -> mnesia:write(Rec) end),
...
to_map(#cx_routing_profile{key = {_, Id}, name = Name, max_total = MaxTotal,
                           media_caps = MediaCaps, guards = Guards}) ->
    #{<<"id">> => Id, <<"name">> => Name,
      <<"max_total">> => case MaxTotal of unlimited -> null; N -> N end,
